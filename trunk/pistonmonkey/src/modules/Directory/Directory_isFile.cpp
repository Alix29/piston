...
JSBool Directory_function_isFile_internal(JSContext * cx, char * path, jsval * rval)
{
	...
	if(result==-1)
	{
		...
		JS_ReportError(cx, "Directory.isFile failed to stat path [%s] Error (%d): %s", path, errno, strerror_r(errno, error_buf, MAX_ERROR_LENGTH));
		return JS_FALSE;
	}
...
LM_FUNCTION(Directory_function_isFile_static, "static Directory.isFile")
	...
	return Directory_function_isFile_internal(cx, path, rval);
}

LM_FUNCTION(Directory_function_isFile, "Directory.isFile")
