...
JSBool File_function_isDirectory_internal(JSContext * cx, char * path, jsval * rval)
{
	...
	if(result==-1)
	{
		...
		JS_ReportError(cx, "File.isDirectory failed to stat path [%s] Error (%d): %s", path, errno, strerror_r(errno, error_buf, MAX_ERROR_LENGTH));
		return JS_FALSE;
	}
...
LM_FUNCTION(File_function_isDirectory_static, "static File.isDirectory")
	...
	return File_function_isDirectory_internal(cx, path, rval);
}

LM_FUNCTION(File_function_isDirectory, "File.isDirectory")
